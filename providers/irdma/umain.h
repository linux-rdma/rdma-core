...
static inline struct irdma_parent_domain *to_iparent_domain(struct ibv_pd *pd)
{
	struct irdma_upd *upd = container_of(pd, struct irdma_upd, ibv_pd);
	return container_of(upd, struct irdma_parent_domain, base_pd);
}
#endif /* IRDMA_UMAIN_H */
