...
out:
	ionic_dbg(ctx, "poll cq %u qp %u cons %u st %u wrid %#lx op %u len %u",
		  cq->cqid, qp->qpid, qp->rq.queue.cons,
			   ...
			   struct ibv_wc *wc)
{
	...
	do {
		...
		ionic_dbg(ctx,
			  ...
			  cq->cqid, qp->qpid, qp->sq.queue.cons,
			       ...
			       struct ionic_v1_wqe *wqe)
{
	...
	ionic_dbg(ctx, "post send %u prod %u", qp->qpid, qp->sq.queue.prod);
				  ...
				  uint32_t qpid)
{
	...
	if (cmb & IONIC_CMB_EXPDB) {
		...
		/* Ring doorbell */
		ionic_dbg(ctx, "dbell qp %u qtype %d val %#lx",
			      ...
			      struct ibv_recv_wr *wr)
{
	...
	ionic_dbg(ctx, "post recv %u prod %u", qp->qpid, qp->rq.queue.prod);
	...
	if (likely(qp->rq.queue.prod != qp->rq.old_prod)) {
		...
		if (qp->rq.cmb_ptr) {
			...
			ionic_dbg(ctx, "dbell qp %u rq val %#lx",
				  qp->qpid, ionic_queue_dbell_val(&qp->rq.queue));
