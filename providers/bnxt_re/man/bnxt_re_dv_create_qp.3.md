...
receive queue (RQ) rings are backed by memory described by
**ibv_alloc_user_buf**(3). This allows the application to manage QP ring
memory directly.
...
**sq_umem_handle** must be a handle returned by **ibv_alloc_user_buf**(3).
**sq_umem_offset** is the page-aligned byte offset into that buffer
...
**rq_umem_handle** must be a handle returned by **ibv_alloc_user_buf**(3)
...
This call requires Linux kernel 7.2 or later when **sq_umem_handle** is
provided; on older kernels it fails.

A QP created this way is destroyed like any other QP, using
**ibv_destroy_qp**(3).
