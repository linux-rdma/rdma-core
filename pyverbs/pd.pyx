...
cdef class ParentDomainInitAttr(PyverbsObject):
    def __init__(self, PD pd not None, ParentDomainContext pd_context=None,
        ...
        :param comp_mask: Bit-mask of optional fields/flags, e.g.
                          IBV_PARENT_DOMAIN_INIT_ATTR_ALLOW_CC_UNPROTECTED_ALLOC.
                          The ALLOCATORS and PD_CONTEXT bits are derived from
                          pd_context and must not be requested here directly.
        """
        super().__init__()
        self.pd = pd
        self.init_attr.pd = <v.ibv_pd*>pd.pd
        # These bits are backed by the alloc/free callbacks and pd_context,
        # which are only wired up when pd_context is given (below). Requesting
        # them without it would build an inconsistent attr that providers
        # silently ignore (custom allocators are enabled on alloc && free, not
        # by the bit alone).
